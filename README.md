...
The ultimate goal of this project was to use neural data recorded at the time at which a mouse was shown visual stimuli to predict if they later answered a question about the stimuli (whether or not the contrast was higher on the left or right side) correctly. Because we only have brain data for the time at which the mouse was perceiving the stimuli rather than answering or receiving feedback, my approach to this problem was to try to build a model that looks at the neuron data and tries to predict what the mouse is perceiving (whether it is seeing the left contrast as greater than the right, the right greater than the left, or both contrasts as equal). I trained my model only on instances where the mouse answered correctly to ensure that the experimental conditions were consistent with what the mouse was perceiving. Once my model became good at using the neural data to predict what the mouse saw, I fed the test set into it and compared my model's output for what the mouse perceived to the actual trial conditions. If my model output for what the mouse perceived didn't match the actual trial conditions, I predicted that the mouse got it wrong. If they did match, or the contrasts in the stimuli were equal (in which case any answer would be marked as correct according to the experimental design), I predicted that the mouse got it right. In the following report, I will go over my exploratory data analysis and feature extraction, data integration, the training and testing of the helper model for predicting the experimental condition based on the mouse's neuron data, and the results of the final model which uses the output of the helper model to determine if the mouse answered correctly or incorrectly. 
